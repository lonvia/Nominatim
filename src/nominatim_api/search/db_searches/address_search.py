...
"""
Implementation of search for an address (with housenumber).
"""
...
class AddressSearch(base.AbstractSearch):
    """ Generic search for an address (a query with a housenumber inside).
    """
    SEARCH_PRIO = 1
    ...
    def _inner_search_name_cte(self, conn: SearchConnection,
        ...
        # need a relatively large limit here because we don't know which result
        # will have the housenumber
    ...
    async def lookup(self, conn: SearchConnection,
        ...
        if self.has_address_terms:
            # Only when there are address terms, also look at named POIs.
