...
{% for partition in db.partitions %}
  IF in_partition = {{ partition }} THEN
    FOR r IN
      SELECT place_id, keywords, restrict_keywords, rank_address, rank_search,
      ...
      GROUP BY place_id, keywords, restrict_keywords, rank_address, rank_search, isguess, postcode, centroid
    LOOP
      RETURN NEXT r;
    END LOOP;
    RETURN;
  ...
  IF in_partition = {{ partition }} THEN
      SELECT place_id, keywords, restrict_keywords, rank_address, rank_search,
        ...
        WHERE geometry && ST_Expand(feature, extent)
              ...
              AND token_matches_address(token_info, key, array_cat(keywords, restrict_keywords))
...
CREATE OR REPLACE FUNCTION insertLocationAreaLarge(in_partition INTEGER,
                                                   in_place_id BIGINT,
                                                   in_country_code VARCHAR(2),
                                                   in_keywords INTEGER[],
                                                   in_restrict_keywords INTEGER[],
...
{% for partition in db.partitions %}
  IF in_partition = {{ partition }} THEN
    INSERT INTO location_area_large_{{ partition }}
      (partition, place_id, country_code, keywords, restrict_keywords,
       rank_search, rank_address, isguess, postcode, centroid, geometry)
      (SELECT in_partition, in_place_id, in_country_code, in_keywords, in_restrict_keywords,
