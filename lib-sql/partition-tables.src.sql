...
{% for partition in db.partitions %}
  ...
  CREATE UNIQUE INDEX idx_search_name_{{ partition }}_place_id
    ON search_name_{{ partition }} USING BTREE (place_id) {{db.tablespace.address_index}};
  ...
  CREATE INDEX idx_location_road_{{ partition }}_geometry
    ON location_road_{{ partition }} USING GIST (geometry) {{db.tablespace.address_index}};
  CREATE UNIQUE INDEX idx_location_road_{{ partition }}_place_id
    ON location_road_{{ partition }} USING BTREE (place_id) {{db.tablespace.address_index}};
{% endfor %}
